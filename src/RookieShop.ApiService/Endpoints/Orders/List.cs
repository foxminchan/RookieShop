...
public sealed class List(ISender sender) : IEndpoint<Ok<ListOrdersResponse>, ListOrdersRequest>
{
    public void MapEndpoint(IEndpointRouteBuilder app) =>
        app.MapGet("/orders",
                async (int pageIndex = 1, int pageSize = 0, OrderStatus? status = null, CustomerId? userId = null) =>
                    await HandleAsync(new(pageIndex, pageSize, status, userId)))
    ...
    {
        ListOrdersQuery query = new(request.PageIndex, request.PageSize, request.Status, request.UserId);
